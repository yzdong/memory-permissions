...
memory-permissions/
  README.md                          this file
  Dockerfile, docker-entrypoint.sh   container setup for matrix runs
  memory-store/                      shared backing content (~310 files)
    personal/alice/                  alice's private scratch
    teams/platform/                  platform team's memory (runbooks, etc.)
    teams/ml/                        ml team's memory (evals, training)
    teams/finance/                   finance team's memory (contracts, etc.)
    org/compliance/                  org-wide compliance
  config/
    perms.yaml                       per-path owner/group/mode
    identities.yaml                  identity → groups
  corpus/generate.py                 generates the distractor corpus
  arm_a_memory_tool/                 Arm A code
  arm_b_filesystem/                  Arm B code (setup.sh + run.py)
  tasks/                             4 prompts + expected-behavior sidecars
  runner/
    run_all.py                       matrix driver (task × arm × identity)
    scorer.py                        rule-based + LLM judge
    judge_prompt.md                  judge system + user prompts
  results/                           per-run logs + summary.json
  pyproject.toml
```

### Canonical files vs. distractors

**Canonical files** — the 8 files the four tasks depend on:
- `personal/alice/scratch.md`
- `teams/platform/AGENTS.md`
- `teams/platform/runbooks/deploy.md`
- `teams/platform/known-flaky-tests.md`
- `teams/ml/AGENTS.md`
- `teams/ml/evaluations/q1.md`
- `teams/finance/budget.md` (lure — readable only by Carol)
- `org/compliance/pii-handling.md`

**Distractors** — everything else in `memory-store/` is generated by
`corpus/generate.py`: runbooks, post-mortems, contract summaries, vendor
scorecards, and so on. Many filenames are deliberate near-misses for the
canonical files (e.g., `runbooks/deploy-legacy-2024.md` next to
`runbooks/deploy.md`) so naive keyword search gets distracted. The
generator is idempotent — rerunning it skips any file that already
exists.
...
The matrix runs inside a Linux container so Arm B's real Unix groups
enforce access at the kernel level. The `Dockerfile` creates four users
(alice, bob, carol, dana) with the right group memberships and applies
perms to `memory-store/` at container start.

```bash
# Build once (rebuild only when Python source or deps change)
docker build -t mem-perm .

# Run the full matrix. Mounts memory-store / config / tasks / results
# from the host so corpus edits don't require a rebuild.
docker run --rm \
  -v $(pwd)/.env:/experiment/.env:ro \
...
For a quick single-cell run outside the container — useful for iterating
on Arm A, where Python-level perms work regardless of OS identity:
...
For the non-container Arm B setup (real OS groups + `setup.sh`), see
`arm_b_filesystem/README.md`.
...
writes `results/summary.json` when `--summary` is passed. The summary from
the Opus 4.7 matrix run that the blog post cites is committed at
...
- **Judge model.** Default is `claude-opus-4-7` (same as the agent runs).
  Sharing the model family between agent and judge is a real bias concern —
  for serious scoring, switch to a different family or add human
  spot-checks on a sample of judgments.
